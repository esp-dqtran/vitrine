...
const result: AdvancedSearchResult = {
  ...
  facets: {} as never,
  typeCounts: { app: 0, site: 0, screen: 1, flow: 1, component: 1, pattern: 0 },
  nextCursor: null, hasMore: false, degraded: false,
};
const props = (input: { query?: string; recent?: string[]; initialResult?: AdvancedSearchResult | null }) => ({
  ...input,
  state: { ...defaultSearchState, query: input.query ?? "" },
  onStateChange: () => {},
  onClose: () => {},
  onPreview: () => {},
  onViewAll: () => {},
...
test("groups only top Quick Search results by entity type", () => {
  const html = renderToStaticMarkup(<QuickSearch {...props({ query: "check", initialResult: result })} />);
