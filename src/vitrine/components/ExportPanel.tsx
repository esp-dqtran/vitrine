...
export function ExportPanel({ app, platform = 'web', snapshot, screens = [] }: { app: string; platform?: Platform; snapshot?: DesignSystemSnapshot<EvidenceView> | null; screens?: Screen[] }) {
  ...
  return (
    <section style={{ display: 'grid', gap: 20, paddingTop: 28 }}>
      ...
      <Card padding={5} style={{ border: '1px solid var(--color-border-emphasized)', borderRadius: 16, background: 'var(--color-background-surface)', color: 'var(--color-text-primary)' }}>
        <div style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.08em', color: 'var(--color-text-secondary)' }}>Primary export</div>
        <h3 style={{ margin: '8px 0 6px', fontSize: 22 }}>Figma editable library</h3>
        <p style={{ margin: '0 0 18px', maxWidth: 680, lineHeight: 1.5, color: 'var(--color-text-secondary)' }}>Variable collections, text/effect documentation, auto-layout components, observed variant sets, and source-reference frames in a Figma development plugin.</p>
        <Button label="Export editable Figma library" variant="primary" className="export-panel__primary" isDisabled={busy !== null} isLoading={busy === 'figma'} clickAction={() => run('figma')} style={{ borderRadius: 999 }} />
        <div style={{ marginTop: 10, fontSize: 11.5, color: 'var(--color-text-secondary)' }}>Unzip, create a Figma development plugin to receive its ID, replace its code.js with the exported file, then run it in a blank design file.</div>
      </Card>
