  ...
  return (
    <section style={{ display: 'grid', gap: 20, paddingTop: 28 }}>
      ...
      <Card padding={5} style={{ border: '1px solid var(--color-border-emphasized)', borderRadius: 16, background: 'var(--color-background-surface)', color: 'var(--color-text-primary)' }}>
        ...
        <Button label="Export editable Figma library" variant="primary" isDisabled={busy !== null} isLoading={busy === 'figma'} clickAction={() => run('figma')} style={{ borderRadius: 999 }} />
        <div style={{ marginTop: 10, fontSize: 11.5, color: 'var(--color-text-disabled)' }}>Unzip, create a Figma development plugin to receive its ID, replace its code.js with the exported file, then run it in a blank design file.</div>
      </Card>
