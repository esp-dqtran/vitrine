...
import { Button, EmptyState, TextInput } from "@astryxdesign/core";
import {
  CogIcon,
  StackedIcon,
...
function itemCountLabel(collection: ResearchCollection): string {
  const screens = collection.items.filter(
    ({ kind }) => kind === "screen",
  ).length;
  return screens
...
export function CollectionsWorkspacePage({
  ...
  useEffect(() => {
    ...
    void listCollectionScreens(selected.id)
      .then(({ screens }) => {
        if (active) setSelectedScreens(screens);
      })
      .catch((cause) => {
        if (active) setScreensError((cause as Error).message);
      });
    return () => {
      active = false;
    };
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") setPreviewIndex(null);
        ...
        previewIndex < selectedScreens.length - 1
      )
        setPreviewIndex(previewIndex + 1);
    };
  ...
  const removeSavedScreen = async (itemId: number) => {
    ...
    try {
      ...
      setSelectedScreens(
        (screens) =>
          screens?.filter((screen) => screen.itemId !== itemId) ?? null,
      );
      await refresh();
    } catch (cause) {
  ...
  if (collectionId) {
    const previewScreen =
      previewIndex === null ? undefined : selectedScreens?.[previewIndex];
    const galleryLayout: "web-screens" | "mobile-screens" | undefined =
      selectedScreens?.every(({ screen }) => screen.platform === "web")
        ? "web-screens"
        : selectedScreens?.every(
              ({ screen }) =>
                screen.platform === "ios" || screen.platform === "android",
            )
          ? "mobile-screens"
          : undefined;
    ...
    return (
      <>
        <ReferenceDetailShell
          ...
          loading={
            !loaded ||
            Boolean(selected && selectedScreens === null && !screensError)
          }
        >
          {!loaded ? (
            ...
            <div role="alert">
              <EmptyState
                title="Could not load saved screens"
                description={screensError}
              />
            </div>
            ...
            <section
              className="collections-workspace__screens"
              ...
              <ReferenceGallerySection>
                <ReferenceGalleryGrid minCardWidth={240} layout={galleryLayout}>
                  {selectedScreens.map((savedScreen, index) => (
                    <div
                      key={savedScreen.itemId}
                      className={`app-detail--${savedScreen.screen.platform}`}
                    >
                      <ScreenGridCard
                        ...
                        onRemove={() =>
                          void removeSavedScreen(savedScreen.itemId)
                        }
  ...
  return (
    <>
      ...
      {error ? (
        <p
          role="alert"
          className="projects-workspace__error collections-workspace__error"
        >
          {error}
        </p>
      ) : null}
      ...
      {creating ? (
        <section
          className="collections-workspace__composer"
        ...
        <section className="projects-workspace__section collections-workspace__section">
          ...
          <div className="projects-workspace__grid collections-workspace__grid">
            {visibleCollections.map((collection) => (
              <article
                key={collection.id}
                className="collections-workspace__card"
              >
                <button
                  type="button"
                  className="collections-workspace__card-open"
                  onClick={() =>
                    navigate({
                      name: "collections",
                ...
                >
                  <span
                    className="collections-workspace__cover"
                    aria-hidden="true"
                  >
                    <StackedIcon />
                  </span>
