...
export function createReferralLink(fetcher: Fetcher = apiFetch): Promise<{ url: string }> {
  return fetcher("/api/referrals/link", { method: "POST" }).then(jsonOrError<{ url: string }>);
}

export function loadReferralSummary(fetcher: Fetcher = apiFetch): Promise<ReferralSummaryView> {
  return fetcher("/api/referrals/summary").then(jsonOrError<ReferralSummaryView>);
}

export function activateProMonth(fetcher: Fetcher = apiFetch): Promise<{
  status: "activated";
  expiresAt: string;
  availableMonths: number;
}> {
