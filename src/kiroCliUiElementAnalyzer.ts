import { access, mkdtemp, rm, writeFile } from "node:fs/promises";
import { tmpdir } from "node:os";
import { join, resolve } from "node:path";
import sharp from "sharp";
import {
  extractKiroCliJson,
  runKiroCli,
  type KiroCliRunner,
} from "./kiroCliFeatureDocumentProvider.ts";
import {
  buildUiElementExtractionPrompt,
  parseUiElementScreenExtraction,
  type ScreenPatternOption,
  type UiElementScreenExtraction,
} from "./uiElementExtraction.ts";

export interface KiroCliUiElementConfig {
  binary: string;
  model: string;
  providerModel: string;
  effort: string;
  cwd: string;
  maxOutputBytes: number;
}

export interface KiroCliUiElementAnalyzer {
  model: string;
  analyze(input: {
    body: Uint8Array;
    platform: "ios" | "android" | "web";
    screenPatterns: readonly ScreenPatternOption[];
    signal: AbortSignal;
  }): Promise<UiElementScreenExtraction>;
}

const MAX_IMAGE_DIMENSION = 2_000;
const MAX_INPUT_PIXELS = 40_000_000;

function positiveInteger(value: string | undefined, fallback: number): number {
  if (!value?.trim()) return fallback;
  const parsed = Number(value);
  if (!Number.isSafeInteger(parsed) || parsed < 1) {
    throw new Error("Invalid Kiro UI-element output limit");
  }
  return parsed;
}

export function kiroCliUiElementConfigFromEnvironment(
  environment: NodeJS.ProcessEnv = process.env,
): KiroCliUiElementConfig {
  const model = environment.KIRO_CLI_UI_ELEMENT_MODEL?.trim()
    || environment.KIRO_CLI_FEATURE_DOCUMENT_MODEL?.trim()
    || "gpt-5.6-terra";
  const effort = environment.KIRO_CLI_UI_ELEMENT_EFFORT?.trim()
    || environment.KIRO_CLI_FEATURE_DOCUMENT_EFFORT?.trim()
    || "high";
  if (!/^[a-z0-9][a-z0-9._-]{0,159}$/i.test(model)) {
    throw new Error("Invalid Kiro UI-element model");
  }
  if (!/^(low|medium|high|xhigh|max)$/i.test(effort)) {
    throw new Error("Invalid Kiro UI-element effort");
  }
  return {
    binary: environment.KIRO_CLI_BIN?.trim() || "kiro-cli",
    model,
    providerModel: `kiro-cli:${model}-ui-elements`,
    effort: effort.toLowerCase(),
    cwd: resolve(
      environment.KIRO_CLI_UI_ELEMENT_CWD?.trim()
      || environment.KIRO_CLI_FEATURE_DOCUMENT_CWD?.trim()
      || process.cwd(),
    ),
    maxOutputBytes: positiveInteger(
      environment.KIRO_CLI_UI_ELEMENT_MAX_OUTPUT_BYTES,
      4_000_000,
    ),
  };
}

function argumentsFor(config: KiroCliUiElementConfig, prompt: string): string[] {
  return [
    "chat",
    "--model",
    config.model,
    "--effort",
    config.effort,
    "--no-interactive",
    "--trust-tools=fs_read",
    "--wrap",
    "never",
    prompt,
  ];
}

async function normalizedKiroImage(body: Uint8Array): Promise<Buffer> {
  if (body.byteLength < 1) throw new Error("Kiro UI-element image is empty");
  try {
    return await sharp(body, { limitInputPixels: MAX_INPUT_PIXELS })
      .autoOrient()
      .resize({
        width: MAX_IMAGE_DIMENSION,
        height: MAX_IMAGE_DIMENSION,
        fit: "inside",
        withoutEnlargement: true,
      })
      .png({ palette: false, compressionLevel: 9, adaptiveFiltering: true })
      .toBuffer();
  } catch {
    throw new Error("Kiro UI-element image is invalid or exceeds the safe pixel limit");
  }
}

function promptFor(input: {
  imagePath: string;
  platform: string;
  screenPatterns: readonly ScreenPatternOption[];
  validationError?: string;
}): string {
  return [
    `Read the screenshot at this exact absolute path with the read tool: ${input.imagePath}`,
    "Analyze the image pixels, not the filename or surrounding workspace.",
    "Normalized component coordinates must be relative to the complete supplied image.",
    buildUiElementExtractionPrompt(input.platform, input.screenPatterns),
    input.validationError
      ? `Your previous response failed validation: ${input.validationError}\nReturn corrected raw JSON only.`
      : "",
  ].filter(Boolean).join("\n\n");
}

export function createKiroCliUiElementAnalyzer(
  environment: NodeJS.ProcessEnv = process.env,
  runner: KiroCliRunner = runKiroCli,
): KiroCliUiElementAnalyzer {
  const config = kiroCliUiElementConfigFromEnvironment(environment);
  return {
    model: config.providerModel,
    async analyze(input) {
      const image = await normalizedKiroImage(input.body);
      const directory = await mkdtemp(join(tmpdir(), "astryx-kiro-ui-elements-"));
      const imagePath = join(directory, "source.png");
      try {
        await writeFile(imagePath, image, { mode: 0o600 });
        await access(imagePath);
        let validationError = "";
        for (let attempt = 0; attempt < 2; attempt += 1) {
          const output = await runner({
            binary: config.binary,
            args: argumentsFor(config, promptFor({
              imagePath,
              platform: input.platform,
              screenPatterns: input.screenPatterns,
              ...(validationError ? { validationError } : {}),
            })),
            cwd: config.cwd,
            signal: input.signal,
            maxOutputBytes: config.maxOutputBytes,
          });
          try {
            const parsed = extractKiroCliJson(output, (candidate) =>
              typeof candidate.summary === "string"
              && candidate.screenAnalysis !== undefined
              && Array.isArray(candidate.components)
            );
            return parseUiElementScreenExtraction(parsed, input.screenPatterns);
          } catch (error) {
            validationError = (error as Error).message;
            if (attempt === 1) throw error;
          }
        }
        throw new Error("Kiro UI-element output is invalid");
      } finally {
        await rm(directory, { recursive: true, force: true });
      }
    },
  };
}
