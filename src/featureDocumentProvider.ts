...
export const FEATURE_SYNTHESIS_SYSTEM_PROMPT = [
  ...
  "For multi-state evidence, each user story must have 2 to 4 acceptance criteria. Group its happy path with supported validation, alternate, or failure outcomes under the same user story; never create a separate one-criterion requirement for a state in that same journey.",
  "A one-criterion user story is allowed only when the Flow has one evidence item or only one supportable outcome. Never invent alternate, validation, recovery, API, persistence, analytics, or accessibility behavior merely to fill a section.",
