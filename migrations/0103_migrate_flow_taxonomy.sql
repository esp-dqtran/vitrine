-- Classify the existing canonical Flow catalog against the controlled taxonomy.
-- These are deterministic migration rules over the Flow and its legacy parent,
-- not app-specific labels. Every canonical Flow receives one current taxonomy
-- location so public browsing no longer falls back to the legacy parent groups.

-- This is a one-time, 87k-row historical backfill. It runs in the migration's
-- transaction and has no visible partial state; permit it to finish on the
-- shared database rather than failing mid-classification.
...
FROM (
  VALUES
    ...
    ('content-detail', 'saved-items-collection', 'Saved items / collection', 9),
    ('content-detail', 'other-content-detail', 'Other content detail', 10)
