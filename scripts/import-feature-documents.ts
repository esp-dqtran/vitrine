...
export function featureDescriptionToContent(artifact: FeatureDescriptionArtifact): FeatureDocumentContent {
  ...
  const requirements = artifact.feature.requirements.map((item, index) => {
    const sourceCriterion = artifact.feature.acceptanceCriteria[index] ?? artifact.feature.acceptanceCriteria[0];
    const criterion = sourceCriterion ?? {
      id: `AC-${String(index + 1).padStart(2, "0")}`,
      given: "The user enters the flow",
      when: "The user follows the documented steps",
      then: "The documented outcome is reached",
      evidenceIds: [],
    };
    return {
      ...claim(item.id || `REQ-${String(index + 1).padStart(2, "0")}`, item.evidenceIds.length ? "observed" : "proposed", item.text, item.evidenceIds),
      userStory: `As a user, I want to ${item.text.toLowerCase()} so that I can complete the flow goal.`,
...
async function importArtifact(client: pg.PoolClient, artifact: FeatureDescriptionArtifact, markdown: string, apply: boolean): Promise<ImportResult> {
  ...
  try {
    const existing = await client.query<{ id: number; current_revision_id: number | null; author_type: string | null; evidence_manifest_sha256: string | null }>(
      `SELECT d.id, d.current_revision_id, r.author_type, r.evidence_manifest_sha256
       ...
       FOR UPDATE OF d`, [source.appId, source.platformId, artifact.source.flowId],
    );
    ...
    if (current?.evidence_manifest_sha256 === checksum) {
      await client.query("ROLLBACK");
...
export async function runImport(options: { root: string; apply: boolean; databaseUrl: string }): Promise<ImportResult[]> {
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const results = await runImport({ root, apply, databaseUrl });
