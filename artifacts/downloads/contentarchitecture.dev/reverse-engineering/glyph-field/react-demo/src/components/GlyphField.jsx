...
export function GlyphField({
  ...
  useEffect(() => {
    ...
    const data = GLYPH_FIELD_DATA;
    const fontReady = ensureGeistMono();
    const phraseCharacters = phraseAtlasIndices();
    ...
    function resize() {
      ...
      const rows = Math.max(8, Math.round(height / cellHeight));
      ...
      const modelHeight = Math.max(1, Math.round(modelRows));
      ...
      const startX =
        layout === "bottom" ? Math.round((columns - modelWidth) / 2) : columns - modelWidth;
      const startY =
        layout === "bottom" ? rows - modelHeight : Math.round((rows - modelHeight) / 2);
      ...
      uniforms.uEntranceCenter.value[0] = startX + modelWidth / 2;
      uniforms.uEntranceCenter.value[1] = startY + modelHeight / 2;
  ...
  }, [backgroundColor, color, entrance, interactive, modelLayout, pixelRatio]);
